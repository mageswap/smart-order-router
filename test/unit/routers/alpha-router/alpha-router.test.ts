...
import { Fraction, Percent, TradeType } from '@mageswap/sdk-core';
import { Pair } from '@mageswap/v2-sdk';
import { encodeSqrtRatioX96, Pool, Position } from '@mageswap/v3-sdk';
...
describe('alpha router', () => {
  ...
  describe('exact in', () => {
    ...
    test('succeeds to route and generates calldata on v2 only', async () => {
      ...
      const swap = await alphaRouter.route(
        CurrencyAmount.fromRawAmount(USDC_FANTOM, 10000),
        WRAPPED_NATIVE_CURRENCY[250],
        TradeType.EXACT_INPUT,
        swapParams,
        { ...ROUTING_CONFIG, protocols: [Protocol.V2] }
      );
